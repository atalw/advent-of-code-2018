...
fn main() -> Result<()> {
	...
	let root = parse_tree(&mut stream, len);
...
fn a(root: Node) {
	let flat_nodes = flatten_nodes(root);
...
fn parse_tree(stream: &mut Cursor<Vec<u8>>, len: usize) -> Node {
...
fn flatten_nodes(root: Node) -> Vec<Node> {
	let mut nodes: Vec<Node> = Vec::new();
	
	let n = Node {
		child_count: root.child_count,
		metadata_count: root.metadata_count,
		child_nodes: None,
		metadata_entires: root.metadata_entires.clone()
	};
	nodes.push(n);

	flatten_nodes_helper(root, nodes)
...
fn flatten_nodes_helper(curr: Node, mut nodes: Vec<Node>) -> Vec<Node> {
	...
	for child in curr.child_nodes.unwrap() {
		if child.child_nodes.is_some() {
			let mut child_nodes = flatten_nodes_helper(child.clone(), Vec::new());
			let n = Node {
